...
const StoryDetails = () => {
  ...
  useEffect(() => {
    const loadStoryData = async () => {
      try {
        const response = await fetch("http://localhost:5000/scifi"); // Fetch the entire merged stories file
        if (!response.ok) {
  ...
  return (
    <div className="story-detail">
      <Cover
        img={cover}
        title={"Create Sci-Fi Stories"}
        details={
          "Dive into futuristic worlds filled with advanced technology, space exploration, and the mysteries of the universe. Science fiction lets you imagine life beyond Earth, questioning human existence, ethics, and the unknown."
        }
      />
